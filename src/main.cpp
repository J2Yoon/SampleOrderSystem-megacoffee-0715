...
// 현재는 Phase 4까지 구현된 시료 관리/주문 접수 기능만 콘솔에서 수동으로 검증할 수 있게 배선한다.
//
// DI 조립 순서가 중요하다: OrderController가 SampleController::IsSampleRegistered에 의존하므로
// SampleController가 OrderController보다 먼저 생성되어야 한다(docs_temp/phase_4.md 설계 결정 1 참고).
int main()
{
    SetConsoleOutputCP(CP_UTF8);
    SetConsoleCP(CP_UTF8);
    ...
    View::ConsoleView::PrintTitle("반도체 시료 생산주문관리 시스템 (Phase 4 - 시료 관리 / 주문 접수)");
    View::ConsoleView::PrintLine("승인/거절, 생산, 출고, 모니터링 메뉴는 이후 Phase에서 통합될 예정입니다.");

    bool isRunning = true;
    while (isRunning)
    {
        View::ConsoleView::PrintLine();
        View::ConsoleView::PrintLine("[1] 시료 관리   [2] 주문 접수   [0] 종료");
